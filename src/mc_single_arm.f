...
!     Read in flag for using sieve slit.
      read (chanin,1001) str_line
      write(*,*),str_line(1:last_char(str_line))
      if (.not.rd_int(str_line,tmp_int)) 
...
c     call srand(itime)
      ...
      do Itrial = 1,n_trials
         ...
         if(mod(Itrial,100000).eq.0) write(*,*)'event #: ',
...
!     ----------------------------------------------------------------------------
     ...
     >           ms_flag, wcs_flag, decay_flag, resmult, xtar_init,  
     >           ok_spec, pathlen, 5)
...
C     for spectrometer ntuples
         if(ispec.eq.2) then
            if (store_all.OR.(hut_ntuple.AND.ok_spec)) then
               ...
               shms_hut(3) = dx_fp
               shms_hut(4) = dy_fp
               shms_hut(5) = xtar_init
               shms_hut(6) = ztar_init
               shms_hut(7) = ytar_init
               shms_hut(8) = dpp_init
               shms_hut(9) = dth_init/1000.
               shms_hut(10)= dph_init/1000.
               shms_hut(11)= ztar_recon
               shms_hut(12)= ytar_recon
         ...
         if(ispec.eq.1) then
            if (store_all.OR.(hut_ntuple.AND.ok_spec)) then
               ...
               hms_hut(7) = ytar_init
               hms_hut(8) = dpp_init
               hms_hut(9) = dth_init/1000.
               hms_hut(10)= dph_init/1000.
               hms_hut(11)= ztar_recon
               hms_hut(12)= ytar_recon
               hms_hut(13)= dpp_recon
...
C     Indicate where particles are lost in spectrometer.
     ...
     >        hSTOP_VP1,hSTOP_VP2,hSTOP_VP3,hSTOP_VP4
         ...
         write (chanout,1007)
     >        hSTOP_trials,hSTOP_hut,hSTOP_dc1,hSTOP_dc2,hSTOP_scin,
     >        hSTOP_cal,hSTOP_successes,hSTOP_successes
      endif
