...
C in MEs (if they exist)
	...
	if (length(spectr,class).gt.0.01 .and.
     >		abs(length(spectr,class)-zd).gt.0.01 ) then
	    write(6,*) 'PROBLEM WITH TRANSFORMATION #',class,
     >' for spectrometer #',spectr
	    write(6,*) 'Central path length passed to transp =',
     >zd
	    write(6,*) 'But the comments say that the length =',
     >length(spectr,class)
	endif
...
C Check for decay.
	...
	if (decay_flag .and. .not.dflag) then	!check for decay
	  ...
	  if (zd.lt.0.0d0) write(6,*) 
     >'transp distance<0:  automatic decay! BAD!',zd
...
C Compute COSY sums.
	...
	do i = 1,n_terms(spectr,k)
	  ...
	  do j = 1,5
	    temp = 1.0d0
	    if (expon(spectr,j,i,k).ne.0.) temp = 
     >ray(j)**expon(spectr,j,i,k)
	    term = term*temp
	  enddo
...
!	write(*,*) 'TEST TRANSP: ',k, sum(1), sum(2), sum(3), sum(4), sum(5)
!	pause
...
!	write(*,*) 'TEST TRANSP: ',k, xs,dxdzs,ys,dydzs,delta_z
!	pause
...
!	  file_name='shms/shms2008_for_th_pmag7.dat'
	else if (spectr.eq.6) then
	   write(6,*) 'No LSA for SHMS!'
	   stop
...
! If too many transformations, complain!
	  ...
	  if (kk.gt.max_class) stop 
     >'TRANSP_INIT: too many transformations!'
...
! If flag line is seen, increment transformation counter.
     ...
     >		abs(driftdist(spectr,kk)-length(spectr,kk)).gt.0.01) then
	    write(6,*) 'PROBLEM WITH TRANSFORMATION #',kk,
     >' for spectrometer #',spectr
	    write(6,*) 'Appears to be a pure drift, driftdist=',
     >driftdist(spectr,kk)
	    write(6,*) 'But the comments say that the length =',
     >length(spectr,kk)
	  endif
