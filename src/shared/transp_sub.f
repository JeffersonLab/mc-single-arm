...
! Other order terms require all coeff=0
	    ...
	    if (idummy.eq.0 .and. adrift(spectr,kk)) then
	      ...
	      if (order.eq.1) then
	        if (e1.eq.1) then
		  if (abs(c1-1.) .gt. coeff_min) adrift(spectr,kk)=.false.
		  if (abs(c2-0.) .gt. coeff_min) adrift(spectr,kk)=.false.
		  if (abs(c3-0.) .gt. coeff_min) adrift(spectr,kk)=.false.
		  if (abs(c4-0.) .gt. coeff_min) adrift(spectr,kk)=.false.
		else if (e2.eq.1) then
		  driftdist(spectr,kk)=1000.*c1     !drift distance in cm.
		  if (abs(c2-1.) .gt. coeff_min) adrift(spectr,kk)=.false.
		  if (abs(c3-0.) .gt. coeff_min) adrift(spectr,kk)=.false.
		  if (abs(c4-0.) .gt. coeff_min) adrift(spectr,kk)=.false.
		else if (e3.eq.1) then
		  if (abs(c1-0.) .gt. coeff_min) adrift(spectr,kk)=.false.
		  if (abs(c2-0.) .gt. coeff_min) adrift(spectr,kk)=.false.
		  if (abs(c3-1.) .gt. coeff_min) adrift(spectr,kk)=.false.
		  if (abs(c4-0.) .gt. coeff_min) adrift(spectr,kk)=.false.
     ...
     >					     adrift(spectr,kk)=.false. 
		  if (abs(c4-1.) .gt. coeff_min) adrift(spectr,kk)=.false.
		endif
	      else	!if order.ne.1
