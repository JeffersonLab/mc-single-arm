...
!	logical function rd_int(string,number)
	logical function rd_int(string,Inumber)
C+______________________________________________________________________________
!
...
C ============================= Executable Code ================================
	...
	l = last_char(str1)
!	write(*,*) 'STRING LIBRARY reads ',string,str1,str1(1:l),l
!	read (str1(1:l),'(i)',err=999) number
	read (str1(1:l),*,err=999) testnumber
	Inumber=testnumber+0.1
!	write(*,*) 'STRING LIBRARY result ',testnumber,Inumber
! Try to read word as integer.
	rd_int = .true.				!Success.
999	return
	end
...
C ============================= Executable Code ================================
	...
	if (.not.strip(string,str1)) return	!Return if no words
	l = last_char(str1)
	read (str1(1:l),'(L)',err=999) value	
! Try to read word as logical.
