...
! PHI_SCAT from uniform distribution.
	...
	if (p.lt.50.) write(6,*) 'Momentum passed to musc.f 
     > should be in MeV'

	beta = p / sqrt(m2+p*p)
	theta_sigma = Es/p/beta * sqrt(rad_len) * 
     >              (1+epsilon*log10(rad_len))

! Compute new trajectory angles (units are rad)

c	write(6,*) 'theta_sigma,beta,rad_len,log',theta_sigma,beta,rad_len,log10(rad_len)
