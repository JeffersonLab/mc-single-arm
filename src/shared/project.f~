	subroutine project(x_new,y_new,z_drift,decay_flag,dflag,m2,ph,
     >                    pathlen)
...
C Local declarations.
	...
	real*8 bend_lab, bend_total_x, bend_total_y

C ============================= Executable Code ================================

C Check for decay of particle.

!	bend_lab=-3.0 ! deg in-plane bend angle in lab frame
!        bend_total_y = bend_lab*3.14159/180. ! rough horizontal angle rad in plane
	...
	if (.not.decay_flag .or. dflag) then  !particle has already decayed, just drift
	  ...
	  if (z_drift.le.0) write(6,*) 'drift distance<0:  
     > automatic decay! BAD!'

	  z_decay = -1.*dlen*log(1-grnd())

C Check if the decay is within the drift length (i.e. the drift lenght in
C z, times sqrt(1+dxdzs**2+dydzs**2) to correct for the tue path of the ray.
