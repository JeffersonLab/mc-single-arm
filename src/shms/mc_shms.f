...
c     c	parameter (z_entr = 52.04) ! 6.35 cm in front of Q1
...
c        call project(xt,yt,zdrift,decay_flag,dflag,m2,p,pathlen) !project 
         ...
         if ( abs(xt-x_off) .gt. ( -v_entr/h_entr*abs(yt-y_off)
     >        + 3*v_entr/2 ) ) then
...
c        call project(xt,yt,zdrift,decay_flag,dflag,m2,p,pathlen) !project 
         ...
         if ( abs(xt-x_off) .gt. ( (-v_exit-0.8)/(h_exit+0.8)
     >        *abs(yt-y_off) + 3*(v_exit+0.8)/2 ) ) then
           shmsSTOP_COLL_oct = shmsSTOP_COLL_oct + 1
           shmsSTOP_id = 5
           goto 500
        endif
