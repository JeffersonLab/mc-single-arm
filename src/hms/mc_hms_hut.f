      subroutine mc_hms_hut (m2,p,x_fp,dx_fp,y_fp,dy_fp,ms_flag,
     >     wcs_flag,
...
C     DG C spectrometer exit window, 15 mil Kevlar, 5 mil Mylar (use 20 mil,X0=53.3)
...
C     Wire plane positions, construct hdc_zpos array using these parameters
      ...
      parameter (hdc_del_plane = hdc_thick + hdc_wire_thick +
     >           hdc_cath_thick)
...
C     at aerogel exit, drift to hodoscopes
     ...
     >     xs.gt.(hscin_1y_top+hscin_1x_offset) )
     >   scincount = scincount + 1
