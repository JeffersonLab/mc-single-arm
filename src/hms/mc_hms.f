...
C     By Jixie:  I use the common block from the main file 'mc_single_arm.f'
...
!     Read in transport coefficients.
      ...
      if (first_time_here) then
         ...
         close (unit=chan)
         if (n_classes.ne.12) then
            stop 'MC_HMS, wrong number of transport classes'
         endif
...
!     Check front of apertures.
      ...
      if ( abs(xs-x_off) .gt.
     >     (-v_entr/h_entr*abs(ys-y_off)+3*v_entr/2) ) then
         hSTOP_fAper_oct = hSTOP_fAper_oct + 1
         hSTOP_id = 5
         goto 500
      endif

!     Check back of apertures.
      ...
      if ( abs(xs-x_off) .gt.
     >     (-v_exit/h_exit*abs(ys-y_off)+3*v_exit/2)) then
...
!     Check the exit of the 26.65 inch pipe
      zdrift = z_dip1
      call project(xs,ys,zdrift,decay_flag,dflag,m2,p,pathlen) !project and decay
      if ( ((xs-x_offset_pipes)**2+(ys-y_offset_pipes)**2) .gt.
     >     1145.518 ) then
         hSTOP_VP2 = hSTOP_VP2 + 1
         hSTOP_id = 33
         goto 500
      endif
...
!     and track through the detector hut
      ...
      if (.not.adrift(spectr,12)) then
         write (6,*) 'Transformation #12 is NOT a drift'
      endif
